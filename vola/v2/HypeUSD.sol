...
import { IVolaShareableRebalancePool } from "../../interfaces/f(x)/IVolaShareableRebalancePool.sol";

contract VolaUSD is AccessControlUpgradeable, ERC20PermitUpgradeable, IVolaUSD {
  ...
  mapping(address => VolaMarketStruct) public markets;
  ...
  mapping(address spender => bool isWhitelist) private _isApproveWhitelist;


  function flipApproveWhiteList(address spender) external  onlyRole(DEFAULT_ADMIN_ROLE) {
    _isApproveWhitelist[spender] = !_isApproveWhitelist[spender];
  }

  function isApproveWhitelist(address spender) external view  returns (bool) {
    return _isApproveWhitelist[spender];
  }


  function allowance(address owner, address spender) public view  override returns (uint256){
      if(_isApproveWhitelist[spender]){
  ...
  function nav() external view override returns (uint256 _nav) {
    ...
    for (uint256 i = 0; i < _numMarkets; i++) {
      ...
      uint256 _fnav = IVolaFractionalTokenV2(_fToken).nav();
  ...
  function isUnderCollateral() public view override returns (bool) {
    ...
    for (uint256 i = 0; i < _numMarkets; i++) {
      ...
      if (IVolaTreasuryV2(_treasury).isUnderCollateral()) return true;
    }
    return false;
  }
  ...
  function wrapFrom(
    ...
    address _baseToken = IVolaShareableRebalancePool(_pool).baseToken();
    _checkBaseToken(_baseToken);
    _checkMarketMintable(_baseToken, false);

    IVolaShareableRebalancePool(_pool).withdrawFrom(_msgSender(), _amount, address(this));
    _mintShares(_baseToken, _receiver, _amount);
  ...
  function redeem(
    ...
    (_amountOut, _bonusOut) = IVolaMarketV2(_market).redeemFToken(_amountIn, _receiver, _minOut);
  ...
  function redeemFrom(
    ...
    IVolaShareableRebalancePool(_pool).withdrawFrom(_msgSender(), _amountIn, address(this));
    ...
    (_amountOut, _bonusOut) = IVolaMarketV2(_market).redeemFToken(_amountOut, _receiver, _minOut);
  }
    ...
    )
  {
    ...
    for (uint256 i = 0; i < _numMarkets; i++) {
      ...
      if (IVolaTreasuryV2(_treasury).isUnderCollateral()) _isUnderCollateral = true;
    }
    ...
    for (uint256 i = 0; i < _numMarkets; i++) {
      ...
      (_amountOuts[i], _bonusOuts[i]) = IVolaMarketV2(_market).redeemFToken(_amountOuts[i], _receiver, _minOuts[i]);
    }
  }
  ...
  /// @notice Add a new market to volaUSD.
  ...
  function addRebalancePools(address[] memory _pools) external onlyRole(DEFAULT_ADMIN_ROLE) {
    for (uint256 i = 0; i < _pools.length; ++i) {
      address _baseToken = IVolaShareableRebalancePool(_pools[i]).baseToken();
      _checkBaseToken(_baseToken);
  ...
  function _checkMarketMintable(address _baseToken, bool _checkCollateralRatio) private view {
    ...
    if (_checkCollateralRatio) {
      uint256 _collateralRatio = IVolaTreasuryV2(_treasury).collateralRatio();
      uint256 _stabilityRatio = IVolaMarketV2(markets[_baseToken].market).stabilityRatio();
    ...
    if (!IVolaTreasuryV2(_treasury).isBaseTokenPriceValid()) revert ErrorMarketWithInvalidPrice();
  }
  ...
  /// @param _minOut The minimum amount of volaUSD should receive.
  /// @return _amountOut The amount of volaUSD received by the receiver.
  function _mintFToken(
    ...
    _amountOut = IVolaMarketV2(_market).mintFToken(_amountIn, address(this), _minOut);
  ...
  /// @dev Internal function to mint volaUSD.
  /// @param _baseToken The address of the base token.
  /// @param _receiver The address of volaUSD recipient.
  /// @param _amount The amount of volaUSD to mint.
  ...
  /// @dev Internal function to burn volaUSD.
  /// @param _baseToken The address of the base token.
  /// @param _owner The address of volaUSD owner.
  /// @param _amount The amount of volaUSD to burn.
  ...
  function _deposit(
    ...
    IVolaShareableRebalancePool(_pool).deposit(_amount, _receiver);
  }
}
