...
import { IVolaRebalancePoolRegistry } from "../../interfaces/f(x)/IVolaRebalancePoolRegistry.sol";
import { IVolaReservePool } from "../../interfaces/f(x)/IVolaReservePool.sol";
import { IVolaTreasuryV2 } from "../../interfaces/f(x)/IVolaTreasuryV2.sol";

// solhint-disable max-states-count

contract MarketV2 is AccessControlUpgradeable, ReentrancyGuardUpgradeable, IVolaMarketV2 {
  ...
  function getMintFToken(
    uint256 _baseIn
  ) external view returns (uint256 _fTokenMinted,  uint256 _maxBaseInBeforeSystemStabilityMode,  uint256 _baseInFinal,  uint256 _amountWithoutFee) {
    ...
    ( _maxBaseInBeforeSystemStabilityMode, ) = IVolaTreasuryV2(treasury).maxMintableFToken(_stabilityRatio);
    if (_maxBaseInBeforeSystemStabilityMode > 0) {
      _maxBaseInBeforeSystemStabilityMode = IVolaTreasuryV2(treasury).getWrapppedValue(
        _maxBaseInBeforeSystemStabilityMode
      );
    }

    if (fTokenMintPausedInStabilityMode()) {
      uint256 _collateralRatio = IVolaTreasuryV2(treasury).collateralRatio();
    ...
    _amountWithoutFee = _getDeductFTokenMintFee(_baseIn, _maxBaseInBeforeSystemStabilityMode);

    _fTokenMinted = IVolaTreasuryV2(treasury).getMintFToken(
      IVolaTreasuryV2(treasury).getUnderlyingValue(_amountWithoutFee)
    );

    _baseInFinal = _baseIn;

  }



  function getMintXToken(
    uint256 _baseIn
  ) external view returns (uint256 _xTokenMinted,  uint256 _maxBaseInBeforeSystemStabilityMode,  uint256 _baseInFinal,  uint256 _amountWithoutFee, uint256 _bonus) {
    if (mintPaused()) revert ErrorMintPaused();
    ...
    uint256 _stabilityRatio = stabilityRatio();
    (_maxBaseInBeforeSystemStabilityMode, ) = IVolaTreasuryV2(treasury).maxMintableXToken(_stabilityRatio);
    if (_maxBaseInBeforeSystemStabilityMode > 0) {
      _maxBaseInBeforeSystemStabilityMode = IVolaTreasuryV2(treasury).getWrapppedValue(
        _maxBaseInBeforeSystemStabilityMode
      );
    ...
    _amountWithoutFee = _getDeductXTokenMintFee(_baseIn, _maxBaseInBeforeSystemStabilityMode);

    _xTokenMinted = IVolaTreasuryV2(treasury).getMintXToken(
      IVolaTreasuryV2(treasury).getUnderlyingValue(_amountWithoutFee)
    );
    ...
    if (_bonus > 0 && IVolaRebalancePoolRegistry(registry).totalSupply() == 0) {
      _bonus = IVolaReservePool(reservePool).getRequestBonus(baseToken, _bonus);
    } else {
      _bonus = 0;
    }
    _baseInFinal = _baseIn;
  ...
  function getRedeemFToken(
    uint256 _fTokenIn
  ) external view returns (uint256 _baseOut, uint256 _maxBaseOut, uint256 _maxFTokenInBeforeSystemStabilityMode, uint256 _bonus) {
    ...
    uint256 _stabilityRatio = stabilityRatio();
    ( _maxBaseOut,  _maxFTokenInBeforeSystemStabilityMode) = IVolaTreasuryV2(treasury).maxRedeemableFToken(
      _stabilityRatio
    );
    uint256 _feeRatio = _computeFTokenRedeemFeeRatio(_fTokenIn, _maxFTokenInBeforeSystemStabilityMode);

    _baseOut = IVolaTreasuryV2(treasury).getRedeem(_fTokenIn, 0);
    ...
    if (_bonus > 0 && IVolaRebalancePoolRegistry(registry).totalSupply() == 0) {
      ...
      _bonus = IVolaReservePool(reservePool).getRequestBonus(
        baseToken,
        IVolaTreasuryV2(treasury).getWrapppedValue(_bonus)
      );
    } else {
      _bonus = 0;
    }

    _baseOut = IVolaTreasuryV2(treasury).getWrapppedValue(_baseOut);
  ...
  function mintFToken(
    ...
    // make sure caller is volaUSD, when volaUSD is enabled
    {
      address _volaUSD = volaUSD;
      if (_volaUSD != address(0) && _volaUSD != _msgSender()) revert ErrorCallerNotFUSD();
    }
    _beforeMintFToken();
    ...
    uint256 _stabilityRatio = stabilityRatio();
    (uint256 _maxBaseInBeforeSystemStabilityMode, ) = IVolaTreasuryV2(treasury).maxMintableFToken(_stabilityRatio);
    if (_maxBaseInBeforeSystemStabilityMode > 0) {
      _maxBaseInBeforeSystemStabilityMode = IVolaTreasuryV2(treasury).getWrapppedValue(
        _maxBaseInBeforeSystemStabilityMode
      );
    ...
    _fTokenMinted = IVolaTreasuryV2(treasury).mintFToken(
      IVolaTreasuryV2(treasury).getUnderlyingValue(_amountWithoutFee),
      _recipient
    );
  ...
  function mintXToken(
    ...
    uint256 _stabilityRatio = stabilityRatio();
    (uint256 _maxBaseInBeforeSystemStabilityMode, ) = IVolaTreasuryV2(treasury).maxMintableXToken(_stabilityRatio);
    if (_maxBaseInBeforeSystemStabilityMode > 0) {
      _maxBaseInBeforeSystemStabilityMode = IVolaTreasuryV2(treasury).getWrapppedValue(
        _maxBaseInBeforeSystemStabilityMode
      );
    ...
    _xTokenMinted = IVolaTreasuryV2(treasury).mintXToken(
      IVolaTreasuryV2(treasury).getUnderlyingValue(_amountWithoutFee),
      _recipient
    );
    ...
    if (_bonus > 0 && IVolaRebalancePoolRegistry(registry).totalSupply() == 0) {
      _bonus = IVolaReservePool(reservePool).requestBonus(baseToken, _recipient, _bonus);
    } else {
      _bonus = 0;
    }
  ...
  function redeemFToken(
    ...
    uint256 _stabilityRatio = stabilityRatio();
    (uint256 _maxBaseOut, uint256 _maxFTokenInBeforeSystemStabilityMode) = IVolaTreasuryV2(treasury).maxRedeemableFToken(
      _stabilityRatio
    );
    uint256 _feeRatio = _computeFTokenRedeemFeeRatio(_fTokenIn, _maxFTokenInBeforeSystemStabilityMode);

    _baseOut = IVolaTreasuryV2(treasury).redeem(_fTokenIn, 0, _msgSender());
    // give bonus when redeem fToken
    if (_baseOut < _maxBaseOut) {
      _bonus = _baseOut;
    } else {
    ...
    if (_bonus > 0 && IVolaRebalancePoolRegistry(registry).totalSupply() == 0) {
      (uint256 _defaultRatio, int256 _deltaRatio) = fTokenMintFeeRatio();
      _bonus -= (_bonus * uint256(int256(_defaultRatio) + _deltaRatio)) / FEE_PRECISION; // deduct fee
      _bonus = IVolaReservePool(reservePool).requestBonus(
        baseToken,
        _recipient,
        IVolaTreasuryV2(treasury).getWrapppedValue(_bonus)
      );
  ...
  function redeemXToken(
    ...
    uint256 _stabilityRatio = stabilityRatio();
    uint256 _feeRatio;
    (, uint256 _maxXTokenInBeforeSystemStabilityMode) = IVolaTreasuryV2(treasury).maxRedeemableXToken(_stabilityRatio);
    ...
    _baseOut = IVolaTreasuryV2(treasury).redeem(0, _xTokenIn, _msgSender());
    _baseOut = IVolaTreasuryV2(treasury).getWrapppedValue(_baseOut);
    uint256 _balance = IERC20Upgradeable(baseToken).balanceOf(address(this));
    // consider possible slippage
    if (_balance < _baseOut) {
  ...
  /// @notice Enable volaUSD mint.
  /// @param _volaUSD The address of volaUSD token.
  function enableVolaUSD(address _volaUSD) external onlyRole(DEFAULT_ADMIN_ROLE) {
    if (_volaUSD == address(0)) revert ErrorZeroAddress();

    if (volaUSD == address(0)) volaUSD = _volaUSD;
  }
  ...
  function _getDeductFTokenMintFee(uint256 _baseIn, uint256 _maxBaseInBeforeSystemStabilityMode)
    internal
    view
    returns (uint256 _baseInWithoutFee)
  {
    ...
    _baseInWithoutFee = _getDeductMintFee(_baseIn, _feeRatio0, _feeRatio1, _maxBaseInBeforeSystemStabilityMode);
  }

  function _getDeductXTokenMintFee(uint256 _baseIn, uint256 _maxBaseInBeforeSystemStabilityMode)
    internal
    view
    returns (uint256 _baseInWithoutFee)
  {
  ...
  function _getDeductMintFee(
    uint256 _baseIn,
    uint256 _feeRatio0,
    uint256 _feeRatio1,
    uint256 _maxBaseInBeforeSystemStabilityMode
  ) internal view returns (uint256 _baseInWithoutFee) {
