...
import { IVolaMarket } from "../../interfaces/f(x)/IVolaMarket.sol";
...
import { IVolaTreasury } from "../../interfaces/f(x)/IVolaTreasury.sol";
...
/// @notice To add boost for VolaN, we maintain a time-weighted boost ratio for each user.
...
contract ShareableRebalancePool is MultipleRewardCompoundingAccumulator, IVolaShareableRebalancePool {
  ...
  /// @notice The address of VolaN token.
  address public immutable volan;

  /// @notice The address of Voting Escrow VolaN.
  address public immutable ve;

  /// @notice The address of VotingEscrowHelper contract.
  address public immutable veHelper;

  /// @notice The address of VolaN token minter.
  ...
  /// @inheritdoc IVolaShareableRebalancePool
  ...
  function initialize(
    ...
    baseToken = IVolaTreasury(_treasury).baseToken();
    asset = IVolaTreasury(_treasury).fToken();
    ...
    returns (uint256 _liquidated, uint256 _baseOut)
  {
    ...
    IVolaTreasury _treasury = IVolaTreasury(treasury);
    ...
    (_baseOut, ) = IVolaMarket(_market).redeem(_amount, 0, _wrapper, _minBaseOut);
  ...
  function updateWrapper(address _newWrapper) external onlyRole(DEFAULT_ADMIN_ROLE) {
    if (IVolaTokenWrapper(_newWrapper).src() != baseToken) {
      revert ErrorWrapperSrcMismatch();
    }

    address _oldWrapper = wrapper;
    if (_oldWrapper != address(this) && IVolaTokenWrapper(_oldWrapper).dst() != IVolaTokenWrapper(_newWrapper).dst()) {
  ...
  function _checkpoint(address _account) internal virtual override {
    // fetch VolaN from gauge every 24h
    if (gauge.gauge != address(0) && block.timestamp > uint256(gauge.claimAt) + DAY) {
      uint256 _balance = IERC20Upgradeable(volan).balanceOf(address(this));
      ICurveTokenMinter(minter).mint(gauge.gauge);
      uint256 _minted = IERC20Upgradeable(volan).balanceOf(address(this)) - _balance;
      gauge.claimAt = uint64(block.timestamp);
      _notifyReward(volan, _minted);
    }
      ...
      unchecked {
        ...
        if (fullEarned > 0) {
          ...
          if (fullEarned > boostEarned) {
            ...
            _notifyReward(volan, fullEarned - boostEarned);
          }
        }
      }
    } else {
