...
import { IVolaRebalancePool } from "../../interfaces/f(x)/IVolaRebalancePool.sol";
...
contract RebalancePool is OwnableUpgradeable, IVolaRebalancePool {
  ...
  function updateWrapper(address _newWrapper) external onlyOwner {
    require(IVolaTokenWrapper(_newWrapper).src() == baseToken, "src mismatch");

    address _oldWrapper = wrapper;
    if (_oldWrapper != address(this)) {
      require(IVolaTokenWrapper(_oldWrapper).dst() == IVolaTokenWrapper(_newWrapper).dst(), "dst mismatch");
    }
  ...
  function _claim(
    ...
    address _wrapper = wrapper;
    if (_wrapper != address(this) && _unwrap && _token == IVolaTokenWrapper(_wrapper).dst()) {
      IERC20Upgradeable(_token).safeTransfer(_wrapper, _rewards);
      _rewards = IVolaTokenWrapper(_wrapper).unwrap(_rewards);
      _token = IVolaTokenWrapper(_wrapper).src();
    }
