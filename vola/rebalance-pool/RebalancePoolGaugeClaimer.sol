...
contract RebalancePoolGaugeClaimer is Ownable2Step {
  ...
  /// @dev The address of VolaN token.
  address public immutable VolaN;

  /// @dev The address of VolaN token minter.
  address public immutable TOKEN_MINTER;

  /// @notice The address of VolaN reserve pool.
  ...
  /// @notice Claim pending VolaN from gauge and split to rebalance pools.
  /// @param _receiver The address of incentive receiver.
  function claim(address _receiver) external {
    unchecked {
      // @note We allow donating VolaN to this contract, the incentive should only consider minted VolaN.
      uint256 _balance = IERC20(VolaN).balanceOf(address(this));
      ICurveTokenMinter(TOKEN_MINTER).mint(gauge);
      uint256 _minted = IERC20(VolaN).balanceOf(address(this)) - _balance;
      uint256 _incentive = (_minted * incentiveRatio) / PRECISION;
      _balance += _minted;

      if (_incentive > 0) {
        IERC20(VolaN).safeTransfer(_receiver, _incentive);
        _balance -= _incentive;
      }

      if (_balance > 0) {
        uint256 _ratio = _computeSplitterRatio();
        uint256 _splitterVolaN = (_balance * _ratio) / PRECISION;
        // deposit rewards to rebalance pool splitter
        IERC20(VolaN).safeTransfer(splitter, _splitterVolaN);
        // transfer extra VolaN to reserve pool
        IERC20(VolaN).safeTransfer(reservePool, _balance - _splitterVolaN);
        // split rewards
        IVolaRebalancePoolSplitter(splitter).split(VolaN);
      }
    }
  }
  ...
  function _computeSplitterRatio() internal view returns (uint256 _splitterRatio) {
    ...
    uint256 _leverageRatio = IVolaTreasury(treasury).leverageRatio();
