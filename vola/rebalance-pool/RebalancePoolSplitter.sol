...
contract RebalancePoolSplitter is Ownable2Step, IVolaRebalancePoolSplitter {
  ...
  /// @inheritdoc IVolaRebalancePoolSplitter
  ...
  function split(address _token) external override {
    ...
    for (uint256 i = 0; i < _length; i++) {
      ...
      IVolaRebalancePool(_receiver).depositReward(_token, _amount);
